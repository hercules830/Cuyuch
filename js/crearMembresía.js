function configurarOpcionesMembresía() {
  const selectClass = 'membresia-select';
  const nuevaOpcionInput = document.getElementById('nueva-opcion-input-membresia');
  const agregarOpcionBtn = document.getElementById('agregar-opcion-btn-membresia');
  const eliminarOpcionBtn = document.getElementById('eliminar-opcion-btn-membresia');
  ...
  selects.forEach((select) => {
    ...
    if (localStorage.getItem(`crear-membresia-${selectId}`)) {
      select.innerHTML = localStorage.getItem(`crear-membresia-${selectId}`);
    }
  });
  ...
  function guardarOpciones(select) {
    const selectId = select.getAttribute('id');
    localStorage.setItem(`crear-membresia-${selectId}`, select.innerHTML);
  }
