function configurarOpcionesPrograma() {
    const selectClass = 'programa';
    const nuevaOpcionInput = document.getElementById('nueva-opcion-input-programa');
    const agregarOpcionBtn = document.getElementById('agregar-opcion-btn-programa');
    const eliminarOpcionBtn = document.getElementById('eliminar-opcion-btn-programa');
    ...
    selects.forEach((select) => {
      ...
      if (localStorage.getItem(`crear-programa-${selectId}`)) {
        select.innerHTML = localStorage.getItem(`crear-programa-${selectId}`);
      }
    });
    ...
    function guardarOpciones(select) {
      const selectId = select.getAttribute('id');
      localStorage.setItem(`crear-programa-${selectId}`, select.innerHTML);
    }
