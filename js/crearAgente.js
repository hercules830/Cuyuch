function configurarOpcionesAgente() {
  const selectClass = 'select-agente';
  const nuevaOpcionInput = document.getElementById('nueva-opcion-input-agente');
  const agregarOpcionBtn = document.getElementById('agregar-opcion-btn-agente');
  const eliminarOpcionBtn = document.getElementById('eliminar-opcion-btn-agente');
  ...
  selects.forEach((select) => {
    ...
    if (localStorage.getItem(`crear-agente-${selectName}`)) {
      select.innerHTML = localStorage.getItem(`crear-agente-${selectName}`);
    }
  });
  ...
  dialogDeleteBtn.addEventListener('click', function (event) {
    ...
    checkboxes.forEach((checkbox) => {
      const selectName = checkbox.getAttribute('data-select-name');
  ...
  function guardarOpciones(select) {
    const selectName = select.getAttribute('name');
    localStorage.setItem(`crear-agente-${selectName}`, select.innerHTML);
  }
  ...
  myForm.addEventListener('submit', function (event) {
    ...
    selects.forEach((select) => {
      const selectName = select.getAttribute('name');
      localStorage.setItem(`select-options-${selectName}`, select.innerHTML);
    });
  });
